...
def test_mode_docs_1():
    ...
    # on Python >=3.8, statistics.mode returns the first mode encountered (1) rather than raising, so no error path is taken
    assert mode([1, 2], raise_error_if_no_mode=False) == 1
    # statistics.mode no longer raises for multi-modal input on Python >=3.8, so result_if_no_mode is not used here
    assert mode([1, 2], result_if_no_mode="foo") == 1
